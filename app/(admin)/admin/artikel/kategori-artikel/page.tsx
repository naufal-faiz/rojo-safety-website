...
import { getTotalArticles } from "@/lib/data/article/article";
import { createArticleCategory, updateArticleCategory, softDeleteArticleCategory } from "@/lib/data/article/articleCategoryAction";

type PageProps = { searchParams: Promise<{ search?: string; page?: string; }>; };

export default async function ArticleCategoryPage({ searchParams, }: PageProps) {
    const params = await searchParams;
    const search = params.search ?? "";
    const page = Math.max(1, Number(params.page ?? "1") || 1);
    const limit = 10;

    const [categoryResult, totalArticles, publishedArticles,] = await Promise.all([
        getAllArticleCategories({ search, page, limit, }),
        getTotalArticles(),
    ...
    return (
        <CategoryManager
            ...
            publishedItems={publishedArticles ?? 0}
