...
type ArticlePagination = { page: number; limit: number; totalItems: number; totalPages: number };
...
export default function ArticleListClient({
    ...
    const handleDelete = async (id: string, title: string) => {
        ...
        try {
            ...
            router.refresh(); // data datang dari server sekarang, bukan state lokal
        } catch (err) {
    ...
    return (
        <DataListManager<ArticleItem>
            ...
            emptyDescription="Silakan buat artikel baru atau sesuaikan filter pencarian."
            statusOptions={[
            ...
            columns={[
                { label: "Artikel", className: "px-5 py-3.5" },
                { label: "Kategori" },
                { label: "Status" },
                { label: "Views" },
                ...
                >
                    ...
                    <td className="px-5 py-3.5 text-right whitespace-nowrap">
                        <div className="flex items-center justify-end gap-2">
                            <Link
                                href={`/admin/artikel/preview/${article.slug}`}
                                className="inline-flex items-center gap-1 px-3 py-1 text-xs font-medium text-blue-light-400 hover:text-blue-light-700 bg-blue-light-50 hover:bg-blue-light-100 dark:bg-blue-light-950/40 dark:text-blue-light-400 dark:hover:bg-blue-light-950/60 rounded-lg transition-colors"
                            >
                                <EyeIcon /> Preview
                            </Link>
                            <Link
