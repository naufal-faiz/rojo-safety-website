...
export default function TrainingListClient({
    ...
    const {
        ...
        filteredData: filteredTrainings,
    } = useTableFilter({
        data: trainings,
        searchFields: (t) => [t.title, t.slug, t.category?.name ?? ""],
        statusField: "status",
    });

    const {
        currentPage, setCurrentPage, totalPages,
        paginatedData: paginatedTrainings, totalItems,
    } = usePagination(filteredTrainings, 10);
                    ...
                    <table className="w-full text-left border-collapse text-sm">
                        <thead>
                            <tr className="border-b border-gray-200 bg-gray-50/75 dark:border-gray-800 dark:bg-gray-800/50 text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">
                                <th className="px-5 py-3.5">Training</th>
                                <th className="px-4 py-3.5">Kategori</th>
                                <th className="px-4 py-3.5">Sertifikasi</th>
                            ...
                            {paginatedTrainings.length === 0 ? (
                                <tr>
                                    <td colSpan={6} className="px-5 py-12 text-center text-gray-500 dark:text-gray-400">
                                        <EmptyState
                                            title="Tidak ada training ditemukan"
                                            description="Silakan buat training baru atau sesuaikan filter pencarian."
                                        />
                                    </td>
                                </tr>
                            ) : (
