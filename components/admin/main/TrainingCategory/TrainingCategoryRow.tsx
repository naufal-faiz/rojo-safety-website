...
import { updateArticleCategory, softDeleteArticleCategory } from "@/lib/data/article/articleCategoryAction";

type Category = { id: string; name: string };

const ArticleCategoryRow = ({ sequence, category, onUpdated, onDeleted }: {
    sequence: number;
    category: Category;
    ...
    async function handleSave() {
        ...
        try {
            setIsSaving(true);
            const updatedRes = await updateArticleCategory(category.id, name);
            if (updatedRes.success) {
                onUpdated({ id: category.id, name });
                setIsEditing(false);
            }
        } catch (err) {
            console.error(err);
